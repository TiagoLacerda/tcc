...
namespace spanning_tree
{
    ...
    void generate_sequential(const Graph &graph, const std::function<int(const Graph &tree)> &callback, int lower_bound = 1, bool early_halt = false);
    ...
    void generate(const Graph &graph, const std::function<int(const Graph &tree)> &callback, int lower_bound = 1, bool early_halt = false, int num_threads = 1);
